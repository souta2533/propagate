...
def get_email_from_customer(request: CustomerEmailRequest):
    email_propagate = request.email_propagate
    email_customer = request.email_customer

    # email_customerをDBに保存
    email_customer = save_email_customer(email_propagate, email_customer)
    print(email_customer)

    return {"email_propagate": email_propagate, "email_customer": email_customer}
